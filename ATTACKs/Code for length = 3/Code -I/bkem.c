...
void setup(bkem_system_t *sys, bkem_global_params_t gps) 
{
  ...
  //------------------------------------------------------------
   ...
   element_t d,d1,d2,d3,d4,t,t1,t2,t3,r4,F,FF,d_40,d_41;
   element_init_G2(d_40, gps->pairing);
   element_init_G2(d1, gps->pairing);
   element_pow_zn(d1, gbs->PK->gg, alpha);
   element_init_Zr(t, gps->pairing);
   element_add(t,gbs->t[2],gbs->t[3]);
   element_init_G2(F, gps->pairing);
   element_pow_zn(F, gbs->PK->gg, t);
   element_init_G2(FF, gps->pairing);
   element_init_Zr(r4, gps->pairing);
   element_random(r4);
   element_pow_zn(FF, F, r4);
   element_mul(d_40, d1,FF);
   ...
   element_pow_zn(d_41, gbs->PK->gg ,r4);
...
void get_decryption_key(bkem_global_params_t gps, bkem_system_t gbs, pubkey_t PK)
 ...
 //-----------------------------new decryption priviledge---------------------------------------------
 	element_t C_34,b5;
 	element_init_G1(C_34, gps->pairing);
 	element_init_G1(b5, gps->pairing);
 	element_mul(b5,gbs->C_32,gbs->C_33);
 	element_div(C_34,b5,gbs->C_31);	
 //---------------------------------------------------------------------------------------------------
 	element_t b6,b7,b8,b9,b10;
 	element_init_GT(b6, gps->pairing);
 	pairing_apply(b6, C_34, gbs->SK[1], gps->pairing);
